...
def stap_2_na_ROTEM_geleide_stollingscorrectie(extem_ct, fibtem_a5, extem_a5, gewicht_kg, keuze, keuze_fib, levensbedreigend):
    ...
    cofact_flesjes = 0
    cofact_ml_req = 0.0
    cofact_flesjes_20ml = 0
    cofact_flesjes_10ml = 0
    totaal_flesjes = 0
    ...
    if keuze == "Cofact":
        if totaal_flesjes > 0:
            advies["Cofact"] = (f"{totaal_flesjes} flesje{'s' if totaal_flesjes>1 else ''}: {cofact_flesjes_20ml}×20 ml flesje, {cofact_flesjes_10ml}×10 ml flesje ({cofact_ml_req} ml)")
        else:
